...
5. Construct the JSON payload following the schema in `skills/planning-with-linear/reference.md`. Include `last_synced_at` set to the current ISO 8601 UTC timestamp.
6. Write it via `set-active-linear.sh write '<json>'`.
7. Print a one-line confirmation: project URL + phase count + active phase ID.
